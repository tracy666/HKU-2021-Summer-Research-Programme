...
#     pdb.set_trace()
    ...
    for i, cls in enumerate(indexes):
        try:
            AUROCs.append(roc_auc_score(gt_np[:, i], 1e-10 + pred_np[:, i]))
        except ValueError as error:
            print('Error in computing accuracy for {}.\n Error msg:{}'.format(i, error))
            AUROCs.append(0)
    ...
    metraic = { 'auc':round(np.mean(AUROCs),4), 
              'sen':round(np.mean(Senss),4), 
              'spe':round(np.mean(Specs),4), 
              'acc':round(np.mean(Accus),4),               
